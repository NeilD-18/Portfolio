...
import { s3Client, uploadFileToS3 } from "../utils/s3FileUpload.js";
...
const deleteExperience = async (req, res) => {
    try {
        ...
        if (deletedExperience) {
            ...
            if (s3Key) {
                // Prepare the command to delete the object
                const params = {
...
const addExperience = async (req, res) => {
    try {
        const { order, role, responsibilities, startDate, endDate } = req.body;
        
        console.log('Request body:', req.body);
        console.log('Uploaded file:', req.file);

        // Validate required fields
        if (!order || !role || !responsibilities || !startDate || !endDate) {
...
const updateExperience = async (req, res) => {
    try {
      const { id } = req.params; // Assuming the experience ID is passed as a URL parameter
      const { order, role, responsibilities, startDate, endDate } = req.body;
      ...
      // Check if a new file is uploaded and update the image if necessary
      if (req.file) {
        const newFile = req.file;
  
        // Check if the uploaded image is different from the existing one
        if (!companyPictureKey || companyPictureKey !== experience.companyPicture) {
          const s3Response = await uploadFileToS3(newFile);
          console.log('S3 Upload Response:', s3Response);
          companyPictureKey = s3Response.key; // Update the S3 key
        }
      }
      ...
      experience.companyPicture = companyPictureKey;
...
const getExperiences = async (req, res) => {
    try {
        const experiences = await Experience.find().sort({ 'dateRange.startDate': 1 }); // Sort by 'order' in ascending order
